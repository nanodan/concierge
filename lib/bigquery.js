...
const GEOJSON_FIELD_HINTS = ['geojson', 'geometry', 'geom', 'the_geom'];
...
function buildGeoJsonFeatureCollection(objects, fieldNames) {
  const geoField = findFieldByHints(fieldNames, GEOJSON_FIELD_HINTS);
  ...
  if (!anyGeometry) {
    throw new Error('GeoJSON export requires a geometry/geojson column or latitude/longitude columns');
  }
...
async function serializeResults({ format, columns, rows }) {
  ...
  if (safeFormat === 'geojson') {
    const geojson = buildGeoJsonFeatureCollection(objects, fieldNames);
    return {
      format: safeFormat,
      extension: 'geojson',
