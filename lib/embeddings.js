...
async function loadEmbeddings() {
  try {
    ...
    embeddings = new Map(arr.map(e => [e.id, { text: e.text, vector: e.vector }]));
...
async function backfillEmbeddings(conversations, loadMessages) {
  ...
  if (count > 0) {
    console.log(`[EMBED] Backfill complete: ${count} conversations embedded (${skipped} already had embeddings)`);
  }
}
