...
async function getEmbedder() {
  ...
  console.log(styleText('green', '[EMBED] Loading embedding model (first time may download ~23MB)...'));
  modelLoading = pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2');
  embedder = await modelLoading;
  modelLoading = null;
  console.log(styleText('green', '[EMBED] Embedding model loaded'));
  return embedder;
}
...
async function loadEmbeddings() {
  try {
    ...
    console.log(styleText('green', `[EMBED] Loaded ${embeddings.size} embeddings from disk`));
    // Clean up file if we filtered any invalid entries
    if (valid.length < arr.length) {
      console.log(styleText('yellow', `[EMBED] Cleaned ${arr.length - valid.length} invalid entries`));
      await saveEmbeddings();
    }
  } catch (err) {
    if (err.code !== 'ENOENT') {
      console.error(styleText('red', `[EMBED] Load failed: ${err.message}`));
    }
  }
}
...
async function embedConversation(conv) {
  ...
  if (!conv.id) {
    console.error(styleText('red', '[EMBED] Cannot embed conversation without id'));
    return;
  }
  ...
  try {
    ...
    console.error(styleText('red', `[EMBED] Failed to embed conversation ${conv.id}: ${err.message}`));
  }
}
...
function deleteEmbedding(convId) {
  if (embeddings.has(convId)) {
    ...
    saveEmbeddings().catch(err => {
      console.error(styleText('red', `[EMBED] Save failed after delete: ${err.message}`));
    });
  }
}
...
async function backfillEmbeddings(conversations, loadMessages) {
  ...
  for (const conv of conversations.values()) {
    ...
    if (count % 10 === 0) {
      console.log(styleText('green', `[EMBED] Backfilled ${count} conversations...`));
    }
  ...
  if (count > 0 || skipped > 0) {
    console.log(styleText('green', `[EMBED] Backfill complete: ${count} new, ${skipped} already embedded`));
  }
}
