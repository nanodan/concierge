...
const { MODELS, activeProcesses, generateSummary } = require('./claude');
...
function setupRoutes(app) {
  ...
  app.post('/api/files/upload', async (req, res) => {
    ...
    req.on('end', async () => {
      try {
        ...
        res.json({ path: filePath, filename: safeName });
      } catch (err) {
  ...
  // Update conversation (archive, rename, model, autopilot, pinned, useMemory)
  app.patch('/api/conversations/:id', async (req, res) => {
    ...
    await saveIndex();
    res.json({ ok: true, id: conv.id, name: conv.name, archived: conv.archived, pinned: !!conv.pinned, model: conv.model || 'sonnet', autopilot: conv.autopilot !== false, useMemory: conv.useMemory !== false });
  });
  ...
  // Compress conversation (summarize old messages)
  app.post('/api/conversations/:id/compress', async (req, res) => {
    ...
    // Calculate split point - compress first N% of messages
    ...
    // Check if there are already compressed messages
    const hasCompressedMsgs = toCompress.some(m => m.summarized);
    ...
    try {
      ...
      // Calculate original tokens (estimate)
      ...
      // Rebuild messages array: compressed + summary + kept
      conv.messages = [
        ...conv.messages.slice(0, splitIndex),
        summaryMessage,
        ...toKeep
      ];

      // Clear session to force fresh start with summary
  ...
  app.get('/api/conversations/:id/files/search', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
    ...
    // Use git grep: -n (line numbers), -I (ignore binary), limit output
    // Using --no-color to ensure parseable output
    const result = await runGit(cwd, [
      'grep', '-n', '-I', '--no-color', '-e', query.trim()
    ]);
    ...
    // Parse output format: path:line:content
    ...
    for (const line of lines) {
      ...
      // Match path:line:content (path may contain colons on Windows, but unlikely)
  ...
  // List stashes
  app.get('/api/conversations/:id/git/stash', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
  ...
  // Create stash
  app.post('/api/conversations/:id/git/stash', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
    ...
    // Use push with message if provided, otherwise just stash
  ...
  // Pop stash
  app.post('/api/conversations/:id/git/stash/pop', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
  ...
  app.post('/api/conversations/:id/git/stash/apply', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
  ...
  // Drop stash
  app.post('/api/conversations/:id/git/stash/drop', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
  ...
  app.post('/api/conversations/:id/git/revert', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
    ...
    if (!result.ok) {
      const stderr = result.stderr.toLowerCase();
      if (stderr.includes('conflict')) {
        // Abort the revert to leave repo clean
  ...
  // Reset to a commit
  app.post('/api/conversations/:id/git/reset', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
  ...
  app.post('/api/conversations/:id/git/undo-commit', async (req, res) => {
    const conv = conversations.get(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Not found' });
  ...
  // Get Claude CLI capabilities (skills, commands, agents)
  ...
  // Update a memory (toggle enabled, edit text)
