...
// Error message for macOS permission issues - shown when EPERM/EACCES errors occur
...
function setupRoutes(app) {
  ...
  function isPathWithinCwd(baseCwd, resolvedPath) {
    // Ensure baseCwd has a trailing separator for proper prefix matching
    const normalizedBase = baseCwd.endsWith(path.sep) ? baseCwd : baseCwd + path.sep;
    // Path is within cwd if it equals cwd or starts with cwd/
  ...
  app.get('/api/conversations/:id/git/status', withConversation(async (req, res, conv) => {
    ...
    // Git porcelain v1 format: XY FILENAME
    // X = index (staged) status, Y = worktree (unstaged) status
    // Position 0: staged status, Position 1: unstaged status, Position 3+: filename
    // Status codes: M=modified, A=added, D=deleted, R=renamed, C=copied, U=updated, ?=untracked, !=ignored
    ...
    for (const line of statusResult.stdout.split('\n')) {
      ...
      // Untracked files (both positions show '?')
