...
const SUPPORTED_EXTENSIONS = new Set(['.csv', '.parquet', '.json', '.jsonl', '.geojson', '.tsv']);
...
/**
 * Convert DuckDB-specific types to JSON-serializable values
 * Handles BigInt, Date/Time objects, and other special types
...
function convertValue(value) {
  ...
  // Handle objects (including DuckDB special types)
  if (typeof value === 'object') {
    // Check for DuckDB temporal types by looking for common conversion methods
    // DuckDB date/time types have toDouble(), toString(), or similar methods
    if (typeof value.toString === 'function' && value.constructor && value.constructor.name !== 'Object') {
      // It's a special DuckDB type (DuckDBTimestamp, DuckDBDate, DuckDBTime, etc.)
      ...
      // Try toDouble for numeric representation
      if (typeof value.toDouble === 'function') {
...
async function loadFile(filePath, tableName) {
  ...
  if (ext === '.csv') {
    ...
    // Prefer one row per feature for GeoJSON FeatureCollection payloads.
    sql = `
  ...
  try {
    ...
    // Fallback for non-FeatureCollection GeoJSON payloads (Feature/Geometry objects).
    if (ext !== '.geojson') throw err;
    await conn.run(`DROP TABLE IF EXISTS "${name}"`);
    await conn.run(`CREATE TABLE "${name}" AS SELECT * FROM read_json_auto('${escapedPath}')`);
  }
...
async function query(sql, limit = DEFAULT_QUERY_LIMIT, offset = 0) {
  ...
  // Convert DuckDB types (BigInt, Date, etc.) for JSON serialization
  rows = convertValue(rows);
 ...
 * @param {string} sql - The SQL query
 * @param {string} format - Export format: 'parquet', 'csv', or 'json'
 * @param {string} outputPath - Path to write the file
 * @returns {Promise<{rowCount: number}>}
 */
async function exportQuery(sql, format, outputPath) {
  const { connection: conn } = await getInstance();
  ...
  const copySQL = `COPY (${sql}) TO '${escapedPath}' (FORMAT ${formatUpper})`;

  await conn.run(copySQL);

  // Get row count
  const countResult = await query(`SELECT COUNT(*) FROM (${sql}) AS subq`, 1);
  const rowCount = countResult.rows[0]?.[0] || 0;
