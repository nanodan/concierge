...
async function getUniqueUploadFilename(uploadDir, filename) {
  const safeName = sanitizeFilename(filename || `upload-${Date.now()}`);
  const { base, ext } = splitFilename(safeName);

  let candidate = safeName;
  let counter = 1;
  while (await fileExists(path.join(uploadDir, candidate))) {
    counter += 1;
...
function setupFileRoutes(app) {
  ...
  // Attach existing cwd files into conversation uploads
  app.post('/api/conversations/:id/attachments/from-files', withConversation(async (req, res, conv) => {
    const requestedPaths = Array.isArray(req.body?.paths) ? req.body.paths : null;
    ...
    for (const rawPath of requestedPaths) {
      const originalPath = typeof rawPath === 'string' ? rawPath : '';
      const requestPath = originalPath.trim();
      if (!requestPath) {
        failed.push({ path: originalPath, error: 'Invalid path' });
        continue;
      }

      const sourcePath = path.resolve(baseCwd, requestPath);
      if (!isPathWithinCwd(baseCwd, sourcePath)) {
        failed.push({ path: requestPath, error: 'Access denied' });
        continue;
      }
      ...
      try {
        ...
        if (err?.code === 'ENOENT') {
          failed.push({ path: requestPath, error: 'File not found' });
        } else {
          failed.push({ path: requestPath, error: err.message || 'Unable to read file' });
        }
        continue;
      }

      if (!sourceStat.isFile()) {
        failed.push({ path: requestPath, error: 'Path is not a file' });
        continue;
      }

      if (sourceStat.size > MAX_UPLOAD_SIZE) {
        const maxMB = Math.round(MAX_UPLOAD_SIZE / (1024 * 1024));
        failed.push({ path: requestPath, error: `File too large. Maximum size is ${maxMB}MB.` });
        continue;
      }

      try {
        const sourceName = path.basename(sourcePath);
        const targetName = await getUniqueUploadFilename(uploadDir, sourceName);
        const targetPath = path.join(uploadDir, targetName);
        await fsp.copyFile(sourcePath, targetPath);
        attachments.push({
          path: targetPath,
          filename: targetName,
          url: `/uploads/${convId}/${targetName}`,
        });
      } catch (err) {
        failed.push({ path: requestPath, error: err.message || 'Failed to attach file' });
      }
    }
