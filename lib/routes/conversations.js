...
async function createForkWorktree(sourceCwd, branchHint) {
  ...
  return { ok: true, cwd: worktreePath, branch: branchName, repoRoot };
}

function normalizeScopePath(sourceCwd, repoRoot) {
  if (!sourceCwd || !repoRoot) return '';
  const rel = path.relative(repoRoot, sourceCwd);
  if (!rel || rel === '.' || rel.startsWith('..')) return '';
  return rel.split(path.sep).join('/');
}

function getScopePathspecArgs(scopePath = '') {
  const scoped = String(scopePath || '').trim();
  return scoped ? ['--', scoped] : [];
}

function parseLocalOnlyStatusEntries(statusOutput = '') {
  const values = [];
  const seen = new Set();
...
async function applyGitPatch(cwd, patchText, applyArgs = []) {
  if (!patchText || !patchText.trim()) {
    return { ok: true, stderr: '' };
  }

  return new Promise((resolve) => {
    const proc = spawn('git', ['apply', ...applyArgs, '-'], {
      cwd,
    ...
    proc.on('error', (err) => {
      resolve({ ok: false, stderr: err.message || 'git apply failed' });
    });
...
async function copyLocalPaths(sourceRoot, targetRoot, relativePaths = []) {
  let copiedPathCount = 0;

  for (const relPath of relativePaths) {
    if (!relPath) continue;
    const sourcePath = path.resolve(sourceRoot, relPath);
    const targetPath = path.resolve(targetRoot, relPath);
    if (!isPathInside(sourceRoot, sourcePath) || !isPathInside(targetRoot, targetPath)) continue;
    if (sourcePath === sourceRoot || path.basename(sourcePath) === '.git') continue;
  ...
  return copiedPathCount;
}

async function cleanupFailedWorktree(repoRoot, worktreePath, branchName) {
...
async function mirrorLocalStateToWorktree({ sourceCwd, repoRoot, worktreeCwd }) {
  const scopePath = normalizeScopePath(sourceCwd, repoRoot);
  const scopeArgs = getScopePathspecArgs(scopePath);

  const stagedDiffResult = await runGit(['diff', '--cached', '--binary', ...scopeArgs], repoRoot);
  if (!stagedDiffResult.ok) {
    throw new Error(stagedDiffResult.stderr || 'Failed to read staged changes');
  }

  const unstagedDiffResult = await runGit(['diff', '--binary', ...scopeArgs], repoRoot);
  if (!unstagedDiffResult.ok) {
    throw new Error(unstagedDiffResult.stderr || 'Failed to read unstaged changes');
  }

  const statusResult = await runGit(
    ['status', '--porcelain=1', '-z', '--untracked-files=all', '--ignored=matching', ...scopeArgs],
    repoRoot
  );
  if (!statusResult.ok) {
    throw new Error(statusResult.stderr || 'Failed to read local file status');
  }

  const stagedPatch = stagedDiffResult.stdout || '';
  const unstagedPatch = unstagedDiffResult.stdout || '';

  const stagedApply = await applyGitPatch(worktreeCwd, stagedPatch, ['--3way', '--index']);
  if (!stagedApply.ok) {
    throw new Error(stagedApply.stderr || 'Failed to apply staged changes to worktree');
  }

  const unstagedApply = await applyGitPatch(worktreeCwd, unstagedPatch, ['--3way']);
  if (!unstagedApply.ok) {
    throw new Error(unstagedApply.stderr || 'Failed to apply unstaged changes to worktree');
  }

  const localOnlyPaths = parseLocalOnlyStatusEntries(statusResult.stdout || '');
  const copiedPathCount = await copyLocalPaths(repoRoot, worktreeCwd, localOnlyPaths);

  return {
    scopePath,
    copiedPathCount,
    appliedStagedPatch: !!stagedPatch.trim(),
    appliedUnstagedPatch: !!unstagedPatch.trim(),
  };
}
...
function setupConversationRoutes(app) {
  ...
  app.post('/api/conversations/:id/fork', async (req, res) => {
    ...
    const forkCopyLocalState = req.body?.forkCopyLocalState === true;
    ...
    const isTipFork = fromMessageIndex === source.messages.length - 1;
    const claudeForkSessionId = sourceProvider === 'claude'
      && forkWorkspaceMode === 'same'
      && isTipFork
      ? (getLatestSessionId(source.messages) || source.claudeSessionId || null)
      : null;
    ...
    if (forkWorkspaceMode === 'worktree') {
      ...
      if (forkCopyLocalState) {
        try {
          localStateInfo = await mirrorLocalStateToWorktree({
            sourceCwd: source.cwd,
            repoRoot: worktreeInfo.repoRoot,
            worktreeCwd: worktreeInfo.cwd,
          });
        } catch (err) {
          await cleanupFailedWorktree(worktreeInfo.repoRoot, worktreeInfo.cwd, worktreeInfo.branch);
          return res.status(400).json({ error: err.message || 'Failed to copy local state into worktree' });
        }
      }
    }
    const conversation = {
      ...
      name: forkName,
      cwd: forkCwd,
      forkSourceCwd: forkWorkspaceMode === 'worktree' ? source.cwd : null,
      claudeSessionId: null,
      codexSessionId: null,
      claudeForkSessionId,
      messages,
    ...
    res.json({
      ...
      copiedLocalState: !!localStateInfo,
      copiedLocalPathCount: localStateInfo?.copiedPathCount || 0,
      copiedTrackedChanges: !!(localStateInfo?.appliedStagedPatch || localStateInfo?.appliedUnstagedPatch),
    });
  });
  ...
  app.get('/api/conversations/:id/tree', async (req, res) => {
    ...
    async function resolveGitBranch(cwd) {
      ...
      let branch = result.ok ? String(result.stdout || '').trim() : '';
      if (!branch) branch = '';
    ...
    async function buildNode(nodeId) {
      ...
      const workspaceKind = c.cwd && rootCwd && c.cwd !== rootCwd ? 'worktree' : 'shared';
      const gitBranch = workspaceKind === 'worktree' ? await resolveGitBranch(c.cwd || '') : '';
