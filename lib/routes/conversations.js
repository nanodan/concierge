...
const { getProvider, hasProvider, getAllProviders } = require('../providers');
...
async function validateConversationCreate(body) {
  const { name, cwd, model, provider } = body;
  ...
  // For Claude, validate against known models; for Ollama, accept any model
...
function setupConversationRoutes(app) {
  ...
  app.post('/api/conversations', async (req, res) => {
    ...
    const { name, cwd, autopilot, model, sandboxed, provider } = req.body;
    const id = uuidv4();
    const effectiveProvider = provider || 'claude';
    ...
    if (!effectiveModel) {
      effectiveModel = effectiveProvider === 'claude' ? 'claude-sonnet-4.5' : 'llama3.2';
    }
  ...
  app.patch('/api/conversations/:id', withConversation(async (req, res, conv) => {
    ...
    if (req.body.model !== undefined) {
      ...
      // Only validate Claude models; accept any model for Ollama
  ...
  app.post('/api/conversations/:id/compress', async (req, res) => {
    ...
    try {
      ...
      try {
        const provider = getProvider(providerId);
        summary = await provider.generateSummary(toCompress, conv.model || 'claude-sonnet-4.5', conv.cwd);
      } catch {
        // Fallback to default generateSummary
        summary = await generateSummary(toCompress, conv.model || 'claude-sonnet-4.5', conv.cwd);
      }
