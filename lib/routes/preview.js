...
function isLikelyFrontendScript(script) {
  if (!script || typeof script !== 'string') return false;
  const normalized = script.toLowerCase();
  return FRONTEND_SCRIPT_HINTS.some((pattern) => pattern.test(normalized));
}
...
async function detectProjectType(cwd) {
  ...
  try {
    ...
    // Prefer explicit frontend dev scripts.
    if (pkg.scripts?.dev && (isLikelyFrontendScript(pkg.scripts.dev) || hasHtmlFiles)) {
  ...
  // If top-level HTML files are present, prefer static serving so file picker selections are valid.
  if (hasHtmlFiles) {
  ...
  // Fallback to package start script when no static entrypoint was found.
...
function setupPreviewRoutes(app) {
  ...
  app.post('/api/conversations/:id/preview/start', withConversation(async (req, res, conv) => {
    ...
    if (existing) {
      const existingUrl = buildPreviewUrl(existing.port, existing.entryFile);
    ...
    const url = buildPreviewUrl(port, currentFile);
    res.json({
      running: true,
      port,
  ...
  app.get('/api/conversations/:id/preview/status', withConversation(async (req, res, conv) => {
    ...
    const url = buildPreviewUrl(preview.port, currentFile);
    res.json({
      running: true,
      port: preview.port,
      type: preview.type,
